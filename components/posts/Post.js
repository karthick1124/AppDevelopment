...
const Post = ({ post, posts, setPosts }) => {
  ...
  const handleCommentInput = (e) => {
    ...
    const newComment = {
      ...
      profilePic: post.author ? post.author.profileImg.data : '', // Check for null before accessing profileImg
      likes: 0,
      username: post.author ? post.author.fullName : 'Anonymous', // Check for null before accessing fullName
    ...
    <div className='post'>
      <div className='post-header'>
          <div className='post-user' onClick={() => handleFriendsId(post.author?._id)} style={{ cursor: "pointer" }}>
            <img src={post.author?.profileImg?.data || "https://images.pexels.com/photos/1624496/pexels-photo-1624496.jpeg?auto=compress&cs=tinysrgb&w=600"} className='p-img' alt="" />
            <h2>{post.author?.fullName || "Anonymous"}</h2>
      ...
      <p className='body'>{post.description || "No description available"}</p>
      ...
      <p className='body'>{post.content || "No content available"}</p>
