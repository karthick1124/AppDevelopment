...
const Content = () => {
  ...
  const handleCommentInput = async (postId, commentText) => {
    try {
      const response = await axios.post(`http://localhost:3500/posts/${postId}/comments`, { text: commentText });

      // Ensure response and response.data are defined before accessing data
      if (response && response.data) {
        const updatedPost = response.data;
        // Handle the updated post (e.g., update state or UI)
          ...
          <div key={category} className='content__category-section'>
            ...
            <div className='content__items'>
              {sortedPosts[category].slice(0, expandedCategory === category ? undefined : 4).map((post) => (
                <Card key={post.id} className='content_item'>
